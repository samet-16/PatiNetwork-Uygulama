...
const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (password === currentStoredPass || (email && password.length >= 6)) {
      ...
      setIsSuccess(true);
      const loginId = email === 'kullanici@gmail.com' ? 'demo_user_nisa' : `user_${Math.random().toString(36).substr(2, 5)}`;
      const loginName = email === 'kullanici@gmail.com' ? 'Kullanıcı' : email.split('@')[0];
      ...
      <div className={`relative w-full max-w-[340px] z-10 transition-all duration-1000 flex flex-col items-center justify-center ${error ? 'animate-shake' : ''}`}>
        ...
        <div className="w-full bg-white/[0.03] backdrop-blur-3xl rounded-[44px] p-8 border border-white/5 shadow-2xl space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <input type="email" placeholder="kullanici@gmail.com" className="w-full bg-black/40 border border-white/5 rounded-[24px] py-4 px-6 text-sm text-white focus:border-cyan-400/40 outline-none transition-all placeholder:text-zinc-800 font-rounded" value={email} onChange={(e) => setEmail(e.target.value)} />
          </div>
          <div className="space-y-1.5">
